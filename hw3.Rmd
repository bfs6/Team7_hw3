...
states <- matrix(c(state.name, tolower(state.abb)), nrow = length(state.abb))
str_detect(states[,2], loc_state)
statemap <- states[which(str_detect(states[,2], loc_state)==TRUE),]

map("state", col="#01B8AA", bg="#374649", proj="albers", param=c(39,45))
points(mapproject(lq_lat, lq_long), col=NA, bg="#F2C80F", pch=21, cex=dataset$Elevation / elevationMax * 0.5)
map("county", statemap[1], col="#01B8AA", bg="#374649", proj="albers", param=c(39,45))
points(mapproject(lq_lat, lq_long), col=NA, bg="#F2C80F", pch=21, cex=dataset$Elevation / elevationMax * 0.8)
